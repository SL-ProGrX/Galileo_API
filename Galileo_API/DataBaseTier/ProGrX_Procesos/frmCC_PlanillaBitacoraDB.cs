...
namespace Galileo_API.DataBaseTier.ProGrX_Procesos
{
    public class FrmCCPlanillaBitacoraDB
    {
        ...
        public ErrorDto<CcPlanillaBitacoraListaResult> CC_PlanillaBitacora_Lista_Obtener(int CodEmpresa, decimal proceso, string parametros)
        {
            ...
            var response = new ErrorDto<CcPlanillaBitacoraListaResult>
            {
                Code = 0,
                Description = "Ok",
                Result = new CcPlanillaBitacoraListaResult()
            };
            ...
            try
            {
                ...
                var codInstitucion = ExtractKeyFromFiltro(filtros.filtro, FiltroCodInstitucion);
                codInstitucion = (codInstitucion ?? string.Empty).Trim();
                ...
                int offset = fetch > 0 ? pagina * fetch : 0;
            ...
            where B.cod_institucion = @codInstitucion
              and B.proceso = @proceso;";
                ...
                var lista = conn.Query<CcPlanillaBitacoraData>(sql, new
                {
                ...
                lista = sortField switch
                {
                    "id_seq" => sortOrder == 1
                        ? lista.OrderBy(x => x.id_seq).ToList()
                        : lista.OrderByDescending(x => x.id_seq).ToList(),

                    "gestion" => sortOrder == 1
                        ? lista.OrderBy(x => x.gestion).ToList()
                        : lista.OrderByDescending(x => x.gestion).ToList(),

                    "transaccion" => sortOrder == 1
                        ? lista.OrderBy(x => x.transaccion).ToList()
                        : lista.OrderByDescending(x => x.transaccion).ToList(),

                    "documento" => sortOrder == 1
                        ? lista.OrderBy(x => x.documento).ToList()
                        : lista.OrderByDescending(x => x.documento).ToList(),

                    "usuario" => sortOrder == 1
                        ? lista.OrderBy(x => x.usuario).ToList()
                        : lista.OrderByDescending(x => x.usuario).ToList(),

                    "fecha" => sortOrder == 1
                        ? lista.OrderBy(x => x.fecha).ToList()
                        : lista.OrderByDescending(x => x.fecha).ToList(),
                ...
                if (fetch > 0)
                {
                    lista = lista.Skip(offset).Take(fetch).ToList();
                }
