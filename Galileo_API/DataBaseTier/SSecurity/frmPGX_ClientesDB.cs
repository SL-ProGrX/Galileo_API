...
namespace Galileo.DataBaseTier
{
    public class FrmPgxClientesDb
    {
        ...
        private int QuerySpScalarInt(string storedProcedure, object? parameters = null)
        {
            ...
            {
                using var connection = CreateConnection();
                return connection.Query<int>(storedProcedure, parameters, commandType: CommandType.StoredProcedure).FirstOrDefault();
            }
            catch (Exception ex)
            {
                _ = ex.Message;
            ...
            try
            {
                ...
                if (tipo == "desc")
                {
                    ...
                    else
                    {
                        ...
                        info = connection.Query<ClienteDto>(query, new { CodEmpresa }).FirstOrDefault() ?? new ClienteDto();
                    }
                }
                else
                {
        ...
        public ErrorDto Cliente_Modificar(ClienteDto info)
        {
            ...
            try
            {
                ...
                const string updateSql = @"UPDATE PGX_Clientes SET 
        ...
        public RespuestaDto Cliente_Crear(ClienteDto info)
        {
            ...
            try
            {
                ...
                const string sqlMaxCodEmpresa = "SELECT ISNULL(MAX(cod_empresa), 0) + 1 FROM PGX_Clientes";
                var newCodEmpresa = connection.ExecuteScalar<int>(sqlMaxCodEmpresa);

                const string insertSql = @"INSERT INTO PGX_Clientes (
                ...
                Bitacora(new BitacoraInsertarDto
                {
                    EmpresaId = (long)newCodEmpresa,
                    Usuario = info.registro_usuario ?? string.Empty,
                    DetalleMovimiento = "Cliente Id: " + newCodEmpresa,
        ...
        public ErrorDto Cliente_Eliminar(int CodEmpresa, string usuario)
        {
            ...
            try
            {
                ...
                const string query = "delete PGX_Clientes where cod_Empresa = @CodEmpresa";
                connection.Execute(query, new { CodEmpresa });
        ...
        public ErrorDto ContactoCliente_Insertar(ContactoDto contacto)
        {
            try
            {
                ...
                const string getNextCodContactoSql = @"
        ...
        public ErrorDto Clientes_Sincronizar(int CodEmpresa, bool logos)
        {
            ...
            try
            {
                ...
                var clientes = connection.Query<ClienteDto>(sql, new { CodEmpresa }).ToList();
        ...
        {
            const string procedure = "[spPGX_Obtener_Todas_Empresas]";
            return QuerySpList<ClienteDto>(procedure);
        }

        public List<EmpresaServiciosDto> ListaEmpresaContactos(int CodEmpresa)
        {
            const string procedure = "[spPGX_Obtener_Empresa_Servicios]";
            var values = new { Cod_Empresa = CodEmpresa };
            return QuerySpList<EmpresaServiciosDto>(procedure, values);
        }

        public int CrearEmpresa(ClienteDto info)
        {
            const string procedure = "[sp_InsertarPGX_Clientes]";
            var values = new
            {
            ...
            return QuerySpScalarInt(procedure, values);
        }
        ...
        public ErrorDto Cliente_Modificar2(ClienteDto info)
        {
            ...
            try
            {
                ...
                const string procedure = "[spPGX_Actualizar_Datos_Empresa]";
                var values = new
                {
                    COD_EMPRESA = info.cod_empresa,
                    COD_VENDEDOR = info.cod_vendedor,
