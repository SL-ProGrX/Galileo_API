...
{
    public class frmPres_DefinicionDB
    {
        private readonly IConfiguration _config;

        public frmPres_DefinicionDB(IConfiguration config)
        {
            _config = config;
        }
        ...
        public ErrorDto<List<ModeloGenericList>> Pres_Modelo_Unidades_Obtener(int CodEmpresa, string codModelo, int codContab, string usuario)
        {
            ...
            try
            {
                using var connection = new SqlConnection(stringConn);
                {
                    //query = $@"select Cc.INICIO_ANIO,Cc.INICIO_MES, Cc.CORTE_ANIO, Cc.CORTE_MES, Pm.Estado
                    ...
                    //cierre = connection.Query<CntxCierres>(query).FirstOrDefault();
