    ...
    public class FrmInvTiposMarcasDB
    {
        ...
        public ErrorDto<MarcasDataLista> Marcas_Obtener(int CodCliente, int? pagina, int? paginacion, string? filtro)
        {
            var result = DbHelper.WithConn(CreatePortalDb(), CodCliente, connection =>
            {
                ...
                respuesta.Total = connection.QueryFirstOrDefault<int>("SELECT COUNT(*) FROM pv_marcas");

                var parametros = new DynamicParameters();
                var queryBuilder = new StringBuilder(@"SELECT cod_marca,
                                                             descripcion,
                                                             activo
                                                      FROM pv_marcas");
            ...
            return result.Code == 0
                ...
                : DbHelper.CreateErrorResponse(result.Description ?? "Error al obtener marcas.", result.Code.GetValueOrDefault(-1), CrearListaVacia());
        }
        ...
        public ErrorDto<List<MarcasDto>> Marcas_ObtenerTodos(int CodEmpresa)
        {
            var result = DbHelper.ExecuteListQuery<MarcasDto>(
                CreatePortalDb(),
                CodEmpresa,
                "SELECT cod_marca, descripcion, activo FROM pv_marcas ORDER BY cod_marca");
            ...
            return result.Code == 0
                ...
                : DbHelper.CreateErrorResponse(result.Description ?? "Error al obtener todas las marcas.", result.Code.GetValueOrDefault(-1), new List<MarcasDto>());
        }
        ...
        public ErrorDto Marcas_Actualizar(int CodEmpresa, MarcasDto request)
        {
            var result = DbHelper.ExecuteNonQuery(
                CreatePortalDb(),
                CodEmpresa,
                "UPDATE pv_marcas SET descripcion = @Descripcion, activo = @Activo WHERE Cod_Marca = @Cod_Marca",
                new
                {
                    request.Cod_Marca,
                    request.Descripcion,
            ...
            return CrearRespuestaNonQuery(result, "Ok", "Error al actualizar la marca.");
        }
        ...
        public ErrorDto Marcas_Insertar(int CodEmpresa, MarcasDto request)
        {
            var result = DbHelper.ExecuteNonQuery(
                CreatePortalDb(),
                CodEmpresa,
                "INSERT INTO pv_marcas(cod_marca, descripcion, activo) VALUES(@Cod_Marca, @Descripcion, @Activo)",
                new
                {
                    request.Cod_Marca,
                    request.Descripcion,
            ...
            return CrearRespuestaNonQuery(result, "Ok", "Error al insertar la marca.");
        }
        ...
        public ErrorDto Marcas_Eliminar(int CodEmpresa, string marca)
        {
            var result = DbHelper.ExecuteNonQuery(
                CreatePortalDb(),
                CodEmpresa,
                "DELETE pv_marcas WHERE Cod_Marca = @Cod_Marca",
                new { Cod_Marca = marca });

            return CrearRespuestaNonQuery(result, "Ok", "Error al eliminar la marca.");
        }
