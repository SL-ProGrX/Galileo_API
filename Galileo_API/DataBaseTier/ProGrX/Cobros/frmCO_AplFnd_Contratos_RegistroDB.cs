...
{
    public class FrmCOAplFndContratosRegistroDB
    {
        private readonly PortalDB _portalDB;
        private readonly MSecurityMainDb _securityMainDb;

        public FrmCOAplFndContratosRegistroDB(IConfiguration config)
        {
        ...
        /// <summary>
        /// Obtiene el listado principal de contratos para aplicación de fondos a mora.
        ...
        public ErrorDto<List<CoAplFndContratosRegistroListaRow>> CO_AplFnd_Contratos_Registro_Lista_Obtener(
            int codEmpresa,
            CoAplFndContratosRegistroListaRequest request)
        {
            ...
            try
            {
                ...
                const string sql = @"exec spCbr_Fondos_Apl_Contratos_List @Filtro, @Estado, @Lineas;";

                var lista = conn.Query<CoAplFndContratosRegistroListaRow>(
                    sql,
                    new
                    {
                        Filtro = filtro,
            ...
            {
                return DbHelper.CreateErrorResponse<List<CoAplFndContratosRegistroListaRow>>(
                    ex.Message,
                    -1,
                    new List<CoAplFndContratosRegistroListaRow>());
            }
        }
        ...
        public ErrorDto<CoAplFndContratosRegistroData> CO_AplFnd_Contratos_Registro_Obtener(
            int codEmpresa,
            CoAplFndContratosRegistroConsultaRequest request)
        {
            using var conn = DbHelper.OpenConnection(_portalDB, codEmpresa);

            try
            {
                const string sql = @"exec spCbr_Fondos_Apl_Contratos_Consulta @Contrato, @Operacion;";

                var data = conn.QueryFirstOrDefault<CoAplFndContratosRegistroData>(
                    sql,
                    new
                    {
                ...
                if (data is null)
                {
                    return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroData>(
            ...
            {
                return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroData>(ex.Message);
            }
        }
        ...
        public ErrorDto<List<CoAplFndContratosRegistroCreditoRow>> CO_AplFnd_Contratos_Registro_Creditos_Obtener(
            int codEmpresa,
            CoAplFndContratosRegistroCreditosRequest request)
        {
            using var conn = DbHelper.OpenConnection(_portalDB, codEmpresa);
            ...
            try
            {
                ...
                {
                    return DbHelper.CreateErrorResponse<List<CoAplFndContratosRegistroCreditoRow>>(
                        "Debe indicar la identificación.",
                        -1,
                        new List<CoAplFndContratosRegistroCreditoRow>());
                }

                const string sql = @"exec spCbr_Fondos_Apl_Contratos_Persona_Creditos_Activos @Cedula;";

                var dbRows = conn.Query<CoAplFndContratosRegistroCreditoDbRow>(
                    sql,
                    new { Cedula = cedula },
                    commandTimeout: 0).ToList();

                var lista = dbRows.Select(x => new CoAplFndContratosRegistroCreditoRow
                {
        ...
        /// <summary>
        /// Guarda o actualiza un contrato para aplicación de fondos a mora.
            ...
            CoAplFndContratosRegistroGuardarRequest request)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(cedula))
                {
                    return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroGuardarResponse>(
                        "Debe indicar la identificación.");
                }

                const string sql = @"
exec spCbr_Fondos_Apl_Contratos_Add
    @Contrato,
    @Cedula,
    @Firma,
    @Vence,
    ...
    @Usuario;";
                ...
                var dbResp = conn.QueryFirstOrDefault<CoAplFndContratosRegistroGuardarDbResponse>(
                    sql,
                    new
                    {
                        Contrato = request.id_contrato,
                ...
                var result = new CoAplFndContratosRegistroGuardarResponse
                {
                    pass = dbResp.Pass == 1,
            ...
            {
                return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroGuardarResponse>(ex.Message);
            }
        }
        ...
        public ErrorDto<List<CoAplFndContratosRegistroPersonaF4Row>> CO_AplFnd_Contratos_Registro_Personas_F4_Obtener(
            int codEmpresa,
            CoAplFndContratosRegistroPersonaF4Request request)
        {
            using var conn = DbHelper.OpenConnection(_portalDB, codEmpresa);
...
ORDER BY NOMBRE;";
                ...
                var lista = conn.Query<CoAplFndContratosRegistroPersonaF4Row>(
                    sql,
                    new
                    {
                        texto,
            ...
            {
                return DbHelper.CreateErrorResponse<List<CoAplFndContratosRegistroPersonaF4Row>>(
                    ex.Message,
                    -1,
                    new List<CoAplFndContratosRegistroPersonaF4Row>());
            }
        }
            ...
            CoAplFndContratosRegistroCargaLoteRequest request)
        {
            ...
            {
                var usuario = (request.usuario ?? string.Empty).Trim().ToUpperInvariant();
                var detalle = request.detalle ?? new List<CoAplFndContratosRegistroCargaLoteRow>();

                if (string.IsNullOrWhiteSpace(usuario))
                {
                    return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroCargaLoteResponse>(
                ...
                foreach (var item in detalle)
                {
                    ...
                    const string sql = @"
exec spCbr_Fondos_Apl_Contratos_Add_Lote
    @Cedula,
    @Operacion,
    @Notas,
    @Usuario;";
                    ...
                    if (dbResp.Pass != 1)
                    {
                        ...
                        return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroCargaLoteResponse>(
                            ...
                            -1,
                            new CoAplFndContratosRegistroCargaLoteResponse
                            {
                                pass = false,
                                mensaje = dbResp.Mensaje ?? string.Empty,
            ...
            {
                trx.Rollback();
                return DbHelper.CreateErrorResponse<CoAplFndContratosRegistroCargaLoteResponse>(ex.Message);
            }
            catch (Exception ex)
            {
                trx.Rollback();
