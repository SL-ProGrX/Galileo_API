...
{
    public class FrmTesBloqueosDB
    {
        private readonly PortalDB _portalDB;
        private readonly MTesoreria mTesoreria;
        private readonly MSecurityMainDb DBBitacora;

        public FrmTesBloqueosDB(IConfiguration config)
        {
        ...
        public ErrorDto<TesBloqueoTransaccionDto> TES_Bloqueos_Solicitud_Obtener(int CodEmpresa, int Contabilidad, int Solicitud)
        {
            ...
            try
            {
                ...
                var result = conn.Query<TesBloqueoTransaccionDto>(query,
                    new
                    {
                ...
                if (result == null)
                {
                    return DbHelper.CreateErrorResponse<TesBloqueoTransaccionDto>(
                        "No se encontr&oacute; el n&uacute;mero de solicitud, o no se encuentra pendiente");
                }
            ...
            {
               return DbHelper.CreateErrorResponse<TesBloqueoTransaccionDto>(ex.Message);
            }
        }
        ...
        public ErrorDto<TablasListaGenericaModel> TES_Bloqueos_SolicitudesBloquedas_Obtener(int CodEmpresa, string filtros)
        {
            ...
            try
            {
                ...
                response.Result.lista = conn.Query<TesBloqueoTransaccionDto>(query, parametros).ToList();
            }
            catch (Exception ex)
            {
                response.Code = -1;
        ...
        public ErrorDto TES_Bloqueos_Solicitud_Bloquear(int CodEmpresa, int Solicitud, string razon ,string Usuario)
        {
            ...
            try
            {
                ...
                return DbHelper.OkResponse("Solicitud Bloqueada Satisfactoriamente...");
            }
            catch (Exception ex)
            {
        ...
        public ErrorDto TES_Bloqueos_Solicitud_Desbloquear(int CodEmpresa, int Solicitud, string Usuario)
        {
            ...
            try
            {
                var query = @"update Tes_Transacciones set user_hold = Null, fecha_hold = null
                        where nsolicitud = @nsolicitud";
                conn.Execute(query, new { nsolicitud = Solicitud });
                ...
                return DbHelper.OkResponse("Solicitud Des-Bloqueada Satisfactoriamente...");
            }
            catch (Exception ex)
            {
                return DbHelper.ErrorResponse(ex.Message);
