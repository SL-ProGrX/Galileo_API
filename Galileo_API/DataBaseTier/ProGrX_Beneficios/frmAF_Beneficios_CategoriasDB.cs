        ...
        public ErrorDto BeneficiosCategorias_Agregar(int CodEmpresa, BENE_CATEGORIA request)
        {
            ...
            try
            {
                using var connection = new SqlConnection(stringConn);
                {
                    ...
                    {
                        resp.Code = -1;
                        resp.Description = "Ya existe una categor�a con el c�digo: " + request.cod_categoria + ", por favor verifique";
                    }
                    else
                    {
                        int activo = request.activo ? 1 : 0;
        ...
        public ErrorDto registroPermisosCategoria(int CodCliente, string Cod_Categoria, Bene_CategoriaPermisos request)
        {
            ...
            try
            {
                using var connection = new SqlConnection(clienteConnString);
                {
                    ...
                    // Actualizar el estado a su representaci�n textual de cada campo
                    ...
                    {
                        string detalle = $@"El usuario [{request.registro_usuario}] actualiza permiso de Traslado Tesorer�a de [Inactivo] por [Activo] del usuario [{request.nombre}]";
                        RegistrarBitacora(CodCliente, "Actualiza-Web", detalle, Cod_Categoria, request.registro_usuario);
                    }
                    else
                    {
                        string detalle = $@"El usuario [{request.registro_usuario}] actualiza permiso de Traslado Tesorer�a de [Activo] por [Inactivo] del usuario [{request.nombre}]";
                        RegistrarBitacora(CodCliente, "Actualiza-Web", detalle, Cod_Categoria, request.registro_usuario);
                    }
                    }
                    ...
                    {
                        string detalle = $@"El usuario [{request.registro_usuario}] actualiza permiso de Devolver Resoluci�n de [Inactivo] por [Activo] del usuario [{request.nombre}]";
                        RegistrarBitacora(CodCliente, "Actualiza-Web", detalle, Cod_Categoria, request.registro_usuario);
                    }
                    else
                    {
                        string detalle = $@"El usuario [{request.registro_usuario}] actualiza permiso de Devolver Resoluci�n de [Activo] por [Inactivo] del usuario [{request.nombre}]";
                        RegistrarBitacora(CodCliente, "Actualiza-Web", detalle, Cod_Categoria, request.registro_usuario);
                    }
                    }
